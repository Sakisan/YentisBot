...
bot.on('message', message => {
    ...
    if (
        beatmapId ||
        command === '&linkchannel' ||
        settings.linkedChannels.indexOf(message.channel.id) !== -1
    ) {
        if (beatmapId) {
        ...
        switch (command) {
            case '&linkchannel':
                ...
                message.channel.send(
                    'This channel can now be used by the bot.'
                );
            ...
            case '&rebuild':
                ...
                resetSettings();
                createDatabase(beatmapIds).catch(error => {
                    console.warn(error);
                    message.channel.send('Error: ' + error);
                });
                break;
            ...
            case '&continue':
                if (!isOwner(message)) return;
                createDatabase(beatmapIds, settings.curIndex).catch(error =>
                    message.channel.send('Error: ' + error)
                );
                break;
            case '&unlink':
            ...
            case '&rebuildfailed':
                if (!isOwner(message)) return;
                createDatabase(settings.failedIds.slice(0), 0, true).catch(
                    error => message.channel.send('Error: ' + error)
                );
                break;
            case '&stop':
                if (!isOwner(message)) return;
                ...
                if (progressMessage) {
                    message.channel.send(
                        'https://discordapp.com/channels/' +
                            progressMessage.guild.id +
                            '/' +
                            progressMessage.channel.id +
                            '/' +
                            progressMessage.id
                    );
            ...
            case '&scores':
                ...
                getUser(remainder)
                    .then(user => {
                        ...
                        if (amount === 0)
                            message.channel.send(
                                user.username + ' does not have any #1 scores.'
                            );
                        else
                            message.channel.send(
...
function getProgressMessage(channelId) {
    for (let i = 0; i < progressMessages.length; i++) {
        if (progressMessages[i].channel.id === channelId)
            return progressMessages[i];
    }
    return null;
}
...
function createDatabase(ids, startIndex, rebuildFailed) {
    return new Promise((mainResolve, mainReject) => {
        publishMessage(
            'Building: 0.00% (0 of ' + ids.slice(startIndex).length + ')'
        ).then(messages => {
            ...
            let arrays = [],
                size = idList.length / validKeys.length + 1;
            while (idList.length > 0) arrays.push(idList.splice(0, size));

            let index = 0;
            for (let i = 0; i < arrays.length; i++) {
                for (
                    let j = 0, p = Promise.resolve();
                    j < arrays[i].length;
                    j++
                ) {
                    p = p
                        .then(
                            () =>
                                new Promise(resolve => {
                                    ...
                                    settings.curIndex = Math.max(
                                        0,
                                        index - arrays.length + startIndex
                                    );
                                    index++;
                                    doRequest(data, stop);
                                })
...
function doRequest(data, stop) {
    ...
    let realLength = data.rebuildFailed
        ? data.fullArray.length
        : beatmapIds.length;

    console.warn(
        'Processing ' + beatmapId + ' | ' + realIndex + ' of ' + realLength
    );
    ...
    getFirstPlace(beatmapId)
        .then(score => {
            ...
            if (failedIdIndex !== -1)
                settings.failedIds.splice(failedIdIndex, 1);
...
function finishCreatingDatabase(mainResolve) {
    publishMessage(
        'Done. Failed to process ' + settings.failedIds.length + ' maps.'
    );
    progressMessages = [];
    saveFile(DATABASE_FILE, database);
    saveFile(SETTINGS_FILE, settings);
...
function publishMessage(message) {
    ...
    return new Promise((mainResolve, mainReject) => {
        for (
            ...
            i < settings.linkedChannels.length;
            i++
        ) {
            p = p
                .then(
                                ...
                                channel
                                    ...
                                    .then(message => {
                                        ...
                                        if (
                                            ...
                                            settings.linkedChannels.length
                                        )
                                            mainResolve(messages);
                                    })
                                    .catch(() => {
                                        done++;
                                        if (
                    ...
                    requestify
                        .get(
                                ...
                                '/scores',
                            {
                                ...
                                cookies: {
                                    osu_session:
                                        process.env['SESSION_KEY' + keyIndex]
                                }
                            }
                        )
                        .then(response => {
                        ...
                        .catch(error => {
                            ...
                            if (elapsedTime < 1500) {
                                setTimeout(() => {
                                    reject(
                                        'Error: failed to retrieve first place.'
                                    );
                                }, 1500 - elapsedTime);
                            } else {
                                reject(
